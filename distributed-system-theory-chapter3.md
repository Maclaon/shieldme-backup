...
#### 重要点
> + 数据的$$Hash$$函数与机器的$$Hash$$函数是一致的，这一点非常重要
> + 加入虚拟节点的是为了**解决数据倾斜和负载均衡的**的问题，可以将**一台实际的物理机器所对应的多个虚拟节点机器**通过元数据来进行维护。

#### 优缺点
##### 优点
> + 可以任意的动态添加、删除节点，每次添加、删除一个节点仅影响一致性哈希环上相邻的节点。
> + 元信息的量通常比按数据范围分布数据和按数据量分布数据的元数据要小很多
